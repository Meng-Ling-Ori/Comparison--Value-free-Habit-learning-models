...
def generate_bandit_timeseries_habit_rw(trials_train, nr, ns, n_test=1000, p=0.5):
    Rho = np.zeros((trials_train+n_test, nr, ns))
    #no-rewards/rewards(pellets) after actionA/rewards(pellets) after actionB/ rewards(pellets+leisure)/rewards(leisure)
    # state0(initial state)
    Rho[:trials_train,:,0] = [1,0,0]   
    Rho[trials_train:,:,0] = [1,0,0]
    # state1(state after action A)
    Rho[:trials_train,:,1] = [1-p,p,0] #50% no rewards, 50% rewards of action A
    Rho[trials_train:,:,1] = [1,0,0,]  #100% no rewards, 0% rewards of action A
    # state2(state after action B)
    Rho[:trials_train,:,2] = [1,0,0] #100% no rewards, 0% rewards of action B
    Rho[trials_train:,:,2] = [1-p,0,p] #50% no rewards, 50% rewards of action B

    return Rho

def generate_bandit_timeseries_habit_rw_omission(trials_train, nr, ns, n_test=1000, p=0.5):
    Rho = np.zeros((trials_train+n_test, nr, ns))
    #no-rewards/rewards(pellets) after actionA/rewards(pellets) after actionB/ rewards(pellets+leisure)/rewards(leisure)
    # state0(initial state)
    Rho[:trials_train,:,0] = [1,0,0,0,0]   
    Rho[trials_train:,:,0] = [1,0,0,0,0]
    # state1(state after action A)
    Rho[:trials_train,:,1] = [1-p,p,0,0,0] #50% no rewards, 50% rewards of action A
    Rho[trials_train:,:,1] = [1,0,0,0,0]   #100% no rewards, 0% rewards of action A
    # state2(state after action B)
    Rho[:trials_train,:,2] = [0,0,0,0,1]   #100% no rewards, 0% rewards of action B,100%leisure
    Rho[trials_train:,:,2] = [0,0,0,1-p,p] #50% no rewards, 50% rewards of action B,100%leisure
    
    return Rho

def generate_bandit_timeseries_habit_rw_devaluation(trials_train, nr, ns, n_test,p=0.5):
    Rho = np.zeros((trials_train+n_test, nr, ns))
    # state0(initial state)
    Rho[:trials_train,:,0] = [1,0,0,0,0]   
    Rho[trials_train:,:,0] = [1,0,0,0,0]
    # state1(state after action A)
    Rho[:trials_train,:,1] = [1-p,p,0,0,0] #50% no rewards, 50% rewards of action A
    Rho[trials_train:,:,1] = [1-p,p,0,0,0]   #100% no rewards, 0% rewards of action A
    # state2(state after action B)
    Rho[:trials_train,:,2] = [0,0,0,0,1]   #100% no rewards, 0% rewards of action B,100%leisure
    Rho[trials_train:,:,2] = [0,0,0,0,1]   #100%leisure
    
    return Rho    

def generate_bandit_timeseries_habit_retrieval(trials_train, nr, ns, n_test=100, p=0.5):
    Rho = np.zeros((trials_train*2+n_test, nr, ns))
    #no-rewards/rewards(pellets) after actionA/rewards(pellets) after actionB/ rewards(pellets+leisure)/rewards(leisure)
    # state0(initial state)
    Rho[:trials_train,:,0] = [1,0,0]   
    Rho[trials_train:trials_train+n_test,:,0] = [1,0,0] 
    Rho[trials_train+n_test:,:,0] = [1,0,0] 
    # state1(state after action A)
    Rho[:trials_train,:,1] = [1-p,p,0] #50% no rewards, 50% rewards of action A
    Rho[trials_train:trials_train+n_test,:,1] = [1,0,0,]  #100% no rewards, 0% rewards of action A
    Rho[trials_train+n_test:,:,1] = [1-p,p,0]
    # state2(state after action B)
    Rho[:trials_train,:,2] = [1,0,0] #100% no rewards, 0% rewards of action B
    Rho[trials_train:trials_train+n_test,:,2] = [1-p,0,p] #50% no rewards, 50% rewards of action B
    Rho[trials_train+n_test:,:,2] = [1,0,0] 
