...
"""
There are 3 classes here: each corresponding to a framework.
...
import numpy as np
import random
from misc_mbp import gaussian_function as gsf
...
class sim_two_alternative(object):
    def __init__(self, trials, environment,
                 na=2, nm=2, alpha_H=0.001, alpha_R=0.01, alpha_w = 1, 
                 w_g=10, w_h=10, w_0=1, theta_g=5, theta_h=5): 
        ...
        self.alpha_H = alpha_H #step-size parameter, which determines the rate of change(habitual strength)
        self.alpha_R = alpha_R #step-size parameter, determines the rate of change(expectation of reinforcers)
    ...
    def run_agent(self, U0, U_deval, current_trial, 
                  reversal = False, omission = False, devaluation =  False,
                  trials_phase1 = None, trials_phase2 = None): #trials_phase3 = None):
        self.trials_phase1 = trials_phase1
        self.trials_phase2 = trials_phase2
        ...
        if devaluation:
            if t <= (trials_phase1-1): #or t >= trials_phase2 + trials_phase1:
                ...
                U = U_deval
                #U = [0.0095, 0.0095, 0.9608] 
                #U = [0.01942, 0.01942, 0.96117]​
        else:
            U = U0
...
#        self.env.obtained_rewrads(t,U,omission,trials_phase1,self.actions[t])
    ...
    def update_goal_directed_system(self,t,omission,trials_phase1,U):
        ...
        utilities_a1, utilities_a2 = np.zeros(self.nm), np.zeros(self.nm)
        utilities_a1[:2] = [U[0], U[1]]
        utilities_a2[:2] = [U[0], U[2]]
        
        try:
            utilities_a1[2:], utilities_a2[2:] = U[2:], U[2:] 
        except ValueError:
            pass
    
        self.Q[t,0] = np.sum([utilities_a1[i]*self.R[t,0,i] for i in range(self.nm)], axis = 0)
        self.Q[t,1] = np.sum([utilities_a2[i]*self.R[t,1,i] for i in range(self.nm)], axis = 0)
    ...
    def update_arbiter(self,t):
        ...
        if t == 0:
            ...
            w = self.weights[t]
            weight = 1/(1 + np.exp(self.w_g * self.gs[t] - self.w_h * self.hs[t] + self.w_0))
            self.weights[t] =  w + self.alpha_w*(weight - w)
...
class sim_Free_Operant(object):
    def __init__(self, trials, environment, trials_test = 500, 
                 actions_list=np.arange(0,151,1),
                 nm=2, na=2, ni_R=4, ni_H = 29,
...
#        self.effort_rate = np.zeros(trials) # the rates of effort of each trial(=action)
        ...
        for i in range(2,ni_H+2):
            for j,k in enumerate(self.actions_list):
                self.chi[i-2,j] = np.exp( -((k-5*i)**2)/(2*5**2))
...
#        self.effort_rate[t] = self.env.obtained_effort(t,self.action_rate[t]) #/60
        ...
        reinforcer = self.env.obtained_reinforcement(t,self.action_rate[t],VR,VI,omission) #obtained reinforcer
...
#        self.env.obtained_rewrads(t,U)
    ...
    def update_habitual_system(self,t):
        ...
        a_t = np.zeros(self.n_action_rate)
        a_t[tau] = 1
        H_change_list = np.tile(a_t-self.H[t-1],(29,1))
#        gaussian_mean = np.tile(self.actions_list, (29,1))
#        gaussian_x = np.tile(np.arange(10,151,5).reshape(29,1), (1,151))
#        gaussian_sigma = 5
#        gaussian_basis_gradient = gsf(gaussian_x,gaussian_mean,gaussian_sigma)
#        updates = np.sum(self.alpha_H * H_change_list * gaussian_basis_gradient, 1)
        updates = np.sum(self.alpha_H * H_change_list * self.chi, 1)
        self.c_t[t] = self.c_t[t-1] + updates
...
#                    self.c_t[t,i] += self.alpha_H*(0-self.H[t-1,j])*self.chi[i,j]
        self.H[t] = np.dot(self.c_t[t],self.chi)
        #c_t:(trials,ni_H) ; chi(ni_H,n_action_rate)
            
    def update_goal_directed_system(self,t,U):
        tau = np.where(self.actions_list == self.action_rate[t-1])[0][0] #the position of the selected actions rate                       
        r_t = self.reinforcers[t-1]        
        phi_a = self.phi[:,tau]
        
        for r in range(self.nm):
            self.b_t[t,r] = self.b_t[t-1,r] + self.alpha_R * (r_t[r] - self.R[t-1,tau,r]) * phi_a
#            b_t_list = np.tile(self.b_t[t,r], (self.n_action_rate,1)).T
            
            self.R[t,:,r] = np.dot(self.b_t[t,r],self.phi) 
...
#            expected = np.sum(self.P[t-1] * self.R[t,:,0])
#            deviance = (self.R[t,:,0] - expected)**2
#            expected_deviance = np.sum(self.P[t-1] * deviance)
#            self.gs[t] = np.sqrt(expected_deviance)
            self.gs[t] = np.sqrt(np.sum(self.P[t-1]*(self.R[t,:,0] - np.sum(self.P[t-1]*self.R[t,:,0]))**2))
#            self.gs[t] = np.sqrt(np.sum(self.P[t-1]* (self.R[t,:,0] - np.mean(self.R[t,:,0]))**2))
    ...
    def update_pi_a(self,t):  
        ...
        ### update pi(a)
        m_a_function = self.m_a / np.sum(self.m_a)
        P_unnorm =  m_a_function * np.exp(self.D[t]) 
        self.P[t,:] = P_unnorm / np.sum(P_unnorm)
